      ...
      if(!err && res.statusCode === 200) {
        ...
        var hpHandler = new htmlParser.DefaultHandler(function (err, dom) {
          if(!err) {
            ...
            //console.log('dom', JSON.stringify(dom)); // for debug
            //console.log('dom', JSON.stringify(dom[1].children[1], null, 2)); // for debug

            var domBody = (dom && dom[1] && dom[1].children && dom[1].children[1] && dom[1].children[1].type === 'tag' && dom[1].children[1].name === 'body') ? dom[1].children[1] : null;
